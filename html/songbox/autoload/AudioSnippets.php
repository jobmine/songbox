...
// 	private $acceptedTypes = ["image/jpeg", "image/png", "image/gif", "image/tiff", "image/svg+xml"];
	...
	public function store() {
		...
		$clip = new DB\SQL\Mapper($f3->get('db'),$this->pictable);	// create DB query mapper object  ///note:changed 'db'; 'pic' to 'clip' as mapper name
		$clip->picname = $this->filedata["title"];
		$clip->picfile = $this->filedata["name"];
		$clip->pictype = $this->filedata["type"];
		$clip->save();
	}
...
//////EDIT: commented out the file size & accepted type CHECKS ///////
